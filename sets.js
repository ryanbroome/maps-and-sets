...
function filterHashTags(tags) {
  const bannedHashTags = new Set(['nofilter', 'just sayin', 'yolo']);
  return tags.filter((tag) => !bannedHashTags.has(tag));
}

function filterAnnoying(arr) {
  const annoying = ['Bob Thomas', 'Asmir Gutic', 'Chris Yates', 'Betsy'];
  return annoying.filter((name) => arr.has(name));
}
